...
public class XMLParsingActivity extends Activity {
	...
	private void usingXmlResourceParser() {
		...
		try {
			XmlResourceParser mXmlResourceParser = getResources().getXml(
					R.xml.order);
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				if (eventType == XmlPullParser.START_DOCUMENT) {
					...
					if (mXmlResourceParser.getName().equalsIgnoreCase("order")) {
						...
						Log.v(TAG,
								"OrderTabAttributes: "
										+ mXmlResourceParser.getAttributeValue(
												null, "macAddress"));
						Log.v(TAG,
								"OrderTabAttributes: "
										+ mXmlResourceParser.getAttributeValue(
					...
					if (mXmlResourceParser.getName().equalsIgnoreCase(
							"category")) {
						isMyTag = true;
						Log.v(TAG,
	...
	private void usingSAXParser() {
		...
		try {
			...
			mXMLReader.parse(new InputSource(getAssets().open("order.xml")));// Local
																				// from
																				// Asset
																				// folder
		} catch (Exception e) {
	...
	private void usingDOMParser() {
		...
		try {
			URL mURL = new URL(
					"http://joomlavogue.in/staff/Ketan/PhotoUpload/example.xml");
			...
			DocumentBuilder mDocumentBuilder = mDocumentBuilderFactory
					.newDocumentBuilder();
			Document mDocument = mDocumentBuilder.parse(new InputSource(mURL
					.openStream()));// From Internet
			// Document mDocument = mDocumentBuilder.parse(new
			// InputSource(getAssets().open("example.xml")));//From Assets
			...
			for (int i = 0; i < mNodeList.getLength(); i++) {
				...
				mInnerAttributes.add(nameElement
						.getAttribute("sampleattribute"));
				...
				NodeList name2List = mElement
						.getElementsByTagName("tagwithnumber");
